...
class NodeEgoController(Node):
    ...
    def __on_lidar_message(self, data):
        ...
        self._logger.info(min_distance)
        if min_distance<self.min_distance:
            self.speed = 5.0
            if self.buf_min_distance > min_distance: 
                self.steering_angle=30*self.angle_mod
        else:
            self.steering_angle=0.0
            self.speed = 10.0   
        self.buf_min_distance = min_distance
        #pass
    ...
    def drive(self):
        self.__world_model.command_message.speed = self.speed
        self.__world_model.command_message.steering_angle = self.steering_angle
